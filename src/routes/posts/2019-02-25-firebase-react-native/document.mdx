...
we usually prefer to have both codebase in one repo. 
...
To put them in same repo, easiest way is to put them in two separate, isolated folders:
...
Samething goes for header file search paths.
...
These issues are hard to fix and also really hacky, and when `react-native` upgrades, they might break and require manual upgrade.
...
It will keep all the code linted and pretty formatted, and more importantly consistance between all the sub-projects
...
for example, we might need a script to add admin privelege to a certain user.
